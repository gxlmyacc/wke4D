...
function GetInvokeInstance(const MethodPointer: TMethod): TObject;

function GetParams(aObj: TObject; aMethodName: string): TParamInfoArray; overload;
function GetParams(MethodInfoHeader: PMethodInfoHeader): TParamInfoArray; overload;
function GetParamCount(const AParamInfoArray: TParamInfoArray): Integer;  overload;
function GetParamCount(const AMethodInfoHeader: PMethodInfoHeader): Integer; overload;
function GetReturnInfo(aObj: TObject; aMethodName: string): PReturnInfo; overload;
function GetReturnInfo(MethodInfoHeader: PMethodInfoHeader): PReturnInfo; overload;
...
const
  ParamIndexes: array[0..255] of Integer = (
...
begin
  case TypeInfo^.Kind of
    ...
    tkString, tkLString,{$IFDEF DELPHI2009UP}tkUString,{$ENDIF} tkWString, tkInterface, tkClass:
      Result := 4;
    tkMethod, tkInt64:
      Result := 8;
    tkVariant:
...
const
  ...
  cvt  = ckConvert;
  err  = ckError;
  Codes: array[varEmpty..{$IFDEF DELPHI2009UP}varUInt64{$ELSE}varInt64{$ENDIF}, varEmpty..{$IFDEF DELPHI2009UP}varUInt64{$ELSE}varInt64{$ENDIF}] of TConvertKind =
    ...
    {vt_empty}      (none,      err,      err,    err,    err,    err,    err,    err,      err,      err,          err,       err,      none,        err,         err,         err,    err,    err,     err,     err,     err    {$IFDEF DELPHI2009UP},err{$ENDIF}),
    {vt_null}       (err,       none,     err,    err,    err,    err,    err,    err,      err,      err,          err,       err,      none,        err,         err,         err,    err,    err,     err,     err,     err    {$IFDEF DELPHI2009UP},err{$ENDIF}),
    {vt_i2}         (err,       err,      none,   cvt,    cvt,    cvt,    cvt,    cvt,      cvt,      err,          err,       cvt,      none,        err,         cvt,         err,    cvt,    cvt,     cvt,     cvt,     cvt    {$IFDEF DELPHI2009UP},cvt{$ENDIF}),
    {vt_i4}         (err,       err,      none,   none,   cvt,    cvt,    cvt,    cvt,      cvt,      err,          err,       cvt,      none,        err,         cvt,         err,    cvt,    cvt,     cvt,     cvt,     cvt    {$IFDEF DELPHI2009UP},cvt{$ENDIF}),
    {vt_r4}         (err,       err,      cvt,    cvt,    none,   cvt,    cvt,    cvt,      cvt,      err,          err,       cvt,      none,        err,         cvt,         err,    cvt,    cvt,     cvt,     cvt,     cvt    {$IFDEF DELPHI2009UP},cvt{$ENDIF}),
    {vt_r8}         (err,       err,      cvt,    cvt,    cvt,    none,   none,   none,     cvt,      err,          err,       cvt,      none,        err,         cvt,         err,    cvt,    cvt,     cvt,     cvt,     cvt    {$IFDEF DELPHI2009UP},cvt{$ENDIF}),
    {vt_cy}         (err,       err,      cvt,    cvt,    cvt,    none,   none,   none,     cvt,      err,          err,       cvt,      none,        err,         cvt,         err,    cvt,    cvt,     cvt,     cvt,     cvt    {$IFDEF DELPHI2009UP},cvt{$ENDIF}),
    {vt_date}       (err,       err,      cvt,    cvt,    cvt,    none,   none,   none,     cvt,      err,          err,       cvt,      none,        err,         cvt,         err,    cvt,    cvt,     cvt,     cvt,     cvt    {$IFDEF DELPHI2009UP},cvt{$ENDIF}),
    {vt_bstr}       (err,       err,      cvt,    cvt,    cvt,    cvt,    cvt,    cvt,      none,     err,          err,       cvt,      none,        err,         cvt,         err,    cvt,    cvt,     cvt,     cvt,     cvt    {$IFDEF DELPHI2009UP},cvt{$ENDIF}),
    {vt_dispatch}   (err,       err,      err,    err,    err,    err,    err,    err,      err,      none,         err,       err,      none,        none,        err,         err,    err,    err,     err,     err,     err    {$IFDEF DELPHI2009UP},err{$ENDIF}),
    {vt_error}      (err,       err,      err,    err,    err,    err,    err,    err,      err,      err,          none,      err,      none,        err,         err,         err,    err,    err,     err,     err,     err    {$IFDEF DELPHI2009UP},err{$ENDIF}),
    {vt_bool}       (err,       err,      cvt,    cvt,    err,    err,    err,    err,      cvt,      err,          err,       none,     none,        err,         cvt,         err,    cvt,    cvt,     cvt,     cvt,     cvt    {$IFDEF DELPHI2009UP},cvt{$ENDIF}),
    {vt_variant}    (cvt,       cvt,      cvt,    cvt,    cvt,    cvt,    cvt,    cvt,      cvt,      cvt,          cvt,       cvt,      none,        cvt,         cvt,         cvt,    cvt,    cvt,     cvt,     cvt,     cvt    {$IFDEF DELPHI2009UP},cvt{$ENDIF}),
    {vt_unknown}    (err,       err,      err,    err,    err,    err,    err,    err,      err,      err,          err,       err,      none,        none,        err,         err,    err,    err,     err,     err,     err    {$IFDEF DELPHI2009UP},err{$ENDIF}),
    {vt_decimal}    (err,       err,      cvt,    cvt,    cvt,    cvt,    cvt,    cvt,      cvt,      err,          err,       cvt,      none,        err,         none,        err,    cvt,    cvt,     cvt,     cvt,     cvt    {$IFDEF DELPHI2009UP},cvt{$ENDIF}),
    {0x0f }         (err,       err,      err,    err,    err,    err,    err,    err,      err,      err,          err,       err,      none,        err,         err,         none,   err,    err,     err,     err,     err    {$IFDEF DELPHI2009UP},err{$ENDIF}),
    {vt_i1}         (err,       err,      cvt,    cvt,    cvt,    cvt,    cvt,    cvt,      cvt,      err,          err,       cvt,      none,        err,         cvt,         err,    none,   none,    cvt,     cvt,     cvt    {$IFDEF DELPHI2009UP},cvt{$ENDIF}),
    {vt_ui1}        (err,       err,      cvt,    cvt,    cvt,    cvt,    cvt,    cvt,      cvt,      err,          err,       cvt,      none,        err,         cvt,         err,    none,   none,    cvt,     cvt,     cvt    {$IFDEF DELPHI2009UP},cvt{$ENDIF}),
    {vt_ui2}        (err,       err,      none,   cvt,    cvt,    cvt,    cvt,    cvt,      cvt,      err,          err,       cvt,      none,        err,         cvt,         err,    none,   none,    none,    cvt,     cvt    {$IFDEF DELPHI2009UP},cvt{$ENDIF}),
    {vt_ui4}        (err,       err,      none,   none,   cvt,    cvt,    cvt,    cvt,      cvt,      err,          err,       cvt,      none,        err,         cvt,         err,    none,   none,    none,    none,    cvt    {$IFDEF DELPHI2009UP},cvt{$ENDIF}),
    {vt_i8}         (err,       err,      none,   none,   cvt,    cvt,    cvt,    cvt,      cvt,      err,          err,       cvt,      none,        err,         cvt,         err,    none,   none,    none,    none,    none   {$IFDEF DELPHI2009UP},none{$ENDIF})
{$IFDEF DELPHI2009UP}    
...
begin
  ...
  repeat
    ...
    begin
      ...
      begin
        Result := MethodInfo;
        if {$IFDEF DELPHI2009UP}SamePropTypeName{$ELSE}SameText{$ENDIF}(Result^.Name, MethodName) then
          Exit;
        Inc(Integer(MethodInfo), PMethodInfoHeader(MethodInfo)^.Len);
        Dec(Count);
      end;
...
var
  ...
  ParamBytes:    Integer;
  Size:          Integer;
  Frame:         {$IFDEF DELPHI2009UP}PByte{$ELSE}PChar{$ENDIF};
  ResultParam:   Pointer;
...
begin
  ...
  MethodName := {$IFDEF DELPHI2009UP}UTF8ToString({$ENDIF} PMethodInfoHeader(MethodInfo)^.Name{$IFDEF DELPHI2009UP}){$ENDIF};
  ...
  begin
    ...
    if PushData = nil then
      if (Param^.ParamType^.Kind = tkClass) and {$IFDEF DELPHI2009UP}SamePropTypeName{$ELSE}SameText{$ENDIF}(Param^.Name, 'SELF') then
...
function GetParams(MethodInfoHeader: PMethodInfoHeader): TParamInfoArray;
var
  mi:    PMethodInfoHeader;
  miEnd: Pointer;
  param: PParamInfo;
...
function GetParamCount(const AParamInfoArray: TParamInfoArray): Integer; 
var
  Inti: Integer;
begin
  Result:= 0;
  if AParamInfoArray = nil then Exit;
  Result:= Length(AParamInfoArray);
  {$WARNINGS OFF}
  if SameText(AParamInfoArray[0].Name, 'SELF') then Dec(Result);
  {$WARNINGS ON}
  for Inti:= 0 to Length(AParamInfoArray) - 1 do
    if pfResult in AParamInfoArray[Inti].Flags then Dec(Result);
end;

function GetParamCount(const AMethodInfoHeader: PMethodInfoHeader): Integer;
var
  AParamInfoArray: TParamInfoArray;
begin
  AParamInfoArray:= GetParams(AMethodInfoHeader);
  try
    Result := GetParamCount(AParamInfoArray);
  finally
    SetLength(AParamInfoArray, 0);
  end;
end;
...
var
  P: {$IFDEF DELPHI2009UP}PByte{$ELSE}PChar{$ENDIF};
  I: Integer;
begin
  ...
  begin
    Inc(P, 1 + {$IFNDEF DELPHI2009UP}Byte{$ENDIF}(P[1]) + 1);
    Inc(P, {$IFNDEF DELPHI2009UP}Byte{$ENDIF}(P[0]) + 1);
  end;
  if TypeData^.MethodKind = mkFunction then
    // Skip return type name and info
    Inc(P, {$IFNDEF DELPHI2009UP}Byte{$ENDIF}(P[0]) + 1 + 4);
  Result := P;
end;
...
function CreateMethodPointer(const ADynamicInvokeEvent: TDynamicInvokeEvent; TypeData: PTypeData): TMethod; 
begin
